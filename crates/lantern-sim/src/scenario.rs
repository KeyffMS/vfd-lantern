...
pub enum SignalKindV1 {
    ...
    Noise { center: String, amplitude: String },
}
...
pub fn validate_scenario_for_profile(
    ...
    for event in &scenario.document.events {
        match &event.event {
            ScenarioEventV1::ValueChange { parameter_id, value } => {
...
impl SignalDocumentV1 {
    fn values(&self) -> Vec<&str> {
        match &self.signal {
            ...
            SignalKindV1::Ramp { start, step_per_tick } => vec![start, step_per_tick],
            SignalKindV1::FixedSine { center, amplitude, .. }
            | SignalKindV1::Noise { center, amplitude } => vec![center, amplitude],
        }
    }
}
