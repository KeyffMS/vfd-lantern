...
fn main() {
    ...
    let metadata = fs::symlink_metadata(&source)
        .unwrap_or_else(|error| panic!("packaged profile manifest {} is unavailable: {error}", source.display()));
    assert!(
    ...
    let bytes = fs::read(&source)
        .unwrap_or_else(|error| panic!("cannot read packaged profile manifest {}: {error}", source.display()));
