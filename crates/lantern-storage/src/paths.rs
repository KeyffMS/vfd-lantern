...
impl AppPaths {
    ...
    pub fn final_csv_sidecar(csv: &Path) -> PathBuf {
        let mut path: OsString = csv.as_os_str().to_owned();
...
mod tests {
    ...
    #[test]
    fn sidecar_preserves_non_utf8_path_bytes() {
        let csv = PathBuf::from(OsStr::from_bytes(b"/data/capture-\xff.csv"));
        assert_eq!(
            AppPaths::final_csv_sidecar(&csv).as_os_str().as_bytes(),
            b"/data/capture-\xff.csv.session.json"
        );
    }
}
