...
pub(super) fn validate_profile(
    ...
    if let Some(verification) = &document.hardware_verification {
        validate_text("hardware_verification.method", &verification.method, false)?;
        for (index, firmware) in verification.firmware.iter().enumerate() {
            validate_text(
                format!("hardware_verification.firmware[{index}]"),
                firmware,
                false,
            )?;
        }
        if let Some(report) = &verification.qualification_report_id {
            validate_text(
    ...
    validate_parameter_overlaps(&parameters)?;
    document
        .parameters
...
fn validate_parameter_overlaps(
    parameters: &BTreeMap<ParameterId, ValidatedParameter>,
) -> Result<(), ProfileError> {
    for table in [ModbusTable::InputRegisters, ModbusTable::HoldingRegisters] {
        let mut ranges = parameters
            .values()
            .filter(|parameter| parameter.block().table() == table)
            .map(|parameter| {
                (
                    parameter.block().start().get(),
                    parameter.block().end().get(),
                    parameter.id(),
                )
            })
            .collect::<Vec<_>>();
        ranges.sort_by_key(|(start, end, id)| (*start, *end, id.as_str()));
        for pair in ranges.windows(2) {
            ...
            if right_start <= left_end {
                return Err(ProfileError::validation(
                    "parameters",
                    format!(
                        "register overlap between {left_id} ({left_start}..={left_end}) and {right_id} ({right_start}..={right_end})"
                    ),
                ));
            }
        }
