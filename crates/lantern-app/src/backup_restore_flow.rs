...
impl BackupRestoreState {
    ...
    pub(crate) fn view(&self) -> BackupRestoreView {
        BackupRestoreView {
            source_path: self.source_path.as_ref().map(|p| p.to_string_lossy().into_owned()),
            source_backup_id: self.source.as_ref().map(|b| b.backup_id.get()),
            source_complete: self.source.as_ref().is_some_and(BackupSnapshot::is_complete),
            last_capture_path: self.last_capture_path.as_ref().map(|p| p.to_string_lossy().into_owned()),
            current_backup_id: self.current.as_ref().map(|b| b.backup_id.get()),
            ...
            prepared_plan_hash: self.prepared.as_ref().map(|p| p.plan_hash().to_owned()),
            prepared_confirmation: self.prepared.as_ref().map(ApprovedRestorePlan::operator_confirmation_text),
            prepared_steps: self.prepared.as_ref().map(|plan| plan.steps().iter().map(|step| RestoreStepView {
                index: step.index(),
            ...
            prepared_skipped: self.prepared.as_ref().map_or(0, |p| p.skipped().len()),
...
pub enum BackupRestoreEffect {
    Capture { request: Box<BackupCaptureRequest> },
    LoadSource { path: PathBuf },
    PrepareRestore { source: Box<BackupSnapshot>, request: Box<BackupCaptureRequest> },
    ExecuteRestore { plan: ApprovedRestorePlan, confirmation: RestoreConfirmation, snapshot: WriteSessionSnapshot },
}
