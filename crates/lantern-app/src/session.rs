...
pub enum AuditHealth {
    Healthy,
    Degraded { cause: String, since: Instant },
}
...
impl SessionStateMachine {
    ...
    pub fn reduce(
        ...
        match (state, input) {
            ...
            (SessionState::Connecting { .. }, SessionInput::PortOpened { identity }) => transition(
                SessionState::Identifying {
                    opened_port: identity,
                },
                vec![SessionEffect::StartIdentification],
            ),
            (SessionState::Connecting { .. }, SessionInput::PortOpenFailed { .. }) => {
            ...
            (SessionState::Active(active), SessionInput::TransportLost { cause, now }) => {
            ...
            (SessionState::Active(mut active), SessionInput::ReconnectTimerElapsed { now }) => {
            ...
            (SessionState::Active(mut active), SessionInput::RetryNow) => {
                if let Connectivity::Reconnecting {
                    attempt,
                    next_retry_at,
                    last_error,
            ...
            (SessionState::Active(active), SessionInput::ReconnectPortOpened { .. })
            ...
            (SessionState::Active(active), SessionInput::ReconnectFailed { cause, now })
                if matches!(&active.connectivity, Connectivity::Reconnecting { .. }) =>
            {
            ...
            (
                        ...
                        .is_some_and(|identity| same_identity(&active.identity, identity))
                {
                    ...
                    active.authorization =
                        disarmed_for_process(process_writes_enabled, DisarmReason::Reconnected);
            ...
            (
                ...
                active.authorization =
                    disarmed_for_process(process_writes_enabled, DisarmReason::User);
                transition(SessionState::Active(active), Vec::new())
            }
            (SessionState::Active(mut active), SessionInput::ArmingExpired) => {
                active.authorization =
                    disarmed_for_process(process_writes_enabled, DisarmReason::ArmingExpired);
                transition(SessionState::Active(active), Vec::new())
            }
            (SessionState::Active(mut active), SessionInput::IdleDisarmElapsed) => {
                active.authorization =
                    disarmed_for_process(process_writes_enabled, DisarmReason::IdleExpired);
                transition(SessionState::Active(active), Vec::new())
            }
            (
                SessionState::Active(mut active),
            ...
            (SessionState::Active(mut active), SessionInput::WriteFinished { outcome, now })
            ...
            (SessionState::Active(mut active), SessionInput::RestoreAdvanced { next_index }) => {
            ...
            (
                ...
                active.operation = OperationState::Idle;
                active.authorization =
                    disarmed_for_process(process_writes_enabled, DisarmReason::OperationFinished);
                transition(SessionState::Active(active), Vec::new())
            }
            (
                SessionState::Active(mut active),
                ...
                active.authorization =
                    disarmed_for_process(process_writes_enabled, DisarmReason::AuditDegraded);
                active.operation = OperationState::Idle;
                transition(
                    SessionState::Active(active),
                    if had_operation {
            ...
            (SessionState::ShuttingDown, SessionInput::ShutdownComplete) => {
                disconnected(None, Vec::new())
            }
...
mod tests {
    ...
    use crate::{
        AdapterIdentity, AuditHealth, Authorization, BusError, Connectivity, DisarmReason,
        OperationState,
    };
