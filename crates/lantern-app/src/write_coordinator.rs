...
use lantern_domain::{
    ...
    MonotonicInstant, OperationAuditFinish, OperationAuditOutcome, OperationAuditStart,
    OperationId, ParameterAccess, ParameterId, PlanId, RawRegisters, RequestId, RequiredDriveState,
    SessionId, SlaveId, WriteIntent, WriteOutcome,
};
...
impl WriteCoordinator {
    ...
    pub async fn prepare_restore_plan(
        ...
        for step in plan.steps() {
            let parameter = profile.parameter(step.parameter_id()).ok_or_else(|| {
                RestorePlanError::InvalidBackupParameter(step.parameter_id().clone())
            })?;
            let fresh = self
                .read_parameter_raw(parameter, before.session_id, operation_id)
                .await
    ...
    pub async fn begin_restore(
        ...
        for step in plan.steps() {
                    ...
                    .any(|raw| raw == step.target_raw())
            {
                self.session.disarm();
                return Err(
                    RestorePlanError::InvalidBackupParameter(step.parameter_id().clone()).into(),
                );
            }
            let fresh = self
                .read_parameter_raw(parameter, plan.session_id(), plan.operation_id())
    ...
    pub async fn execute_restore_step(
        ...
        {
            self.abort_restore_in_place(permit, "restore-state-changed")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        }
        let profile = match self.trust.active_profile_by_hash(plan.profile_hash()) {
            ...
            _ => {
                self.abort_restore_in_place(permit, "profile-trust-changed")
                    .await;
                return Err(WriteCoordinatorError::InvalidRestorePermit);
            }
        };
        let Some(parameter) = profile.parameter(step.parameter_id()) else {
            self.abort_restore_in_place(permit, "parameter-disappeared")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        };
        if !restore_parameter_allowed(parameter)
            ...
            || parameter
                ...
                .any(|raw| raw == step.target_raw())
        {
            self.abort_restore_in_place(permit, "restore-policy-changed")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        }
        if !matches!(
            ...
            self.abort_restore_in_place(permit, "drive-not-stopped")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        }
        let final_old = match self
            ...
            .await
        {
            ...
            _ => {
                self.abort_restore_in_place(permit, "expected-old-changed")
                    .await;
                return Err(WriteCoordinatorError::InvalidRestorePermit);
            }
        };
        let Some(old_engineering) = parameter.codec().decode(final_old.as_slice()).ok() else {
            self.abort_restore_in_place(permit, "old-value-decode-failed")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        };
        let Some(target_engineering) = parameter.codec().decode(step.target_raw().as_slice()).ok()
        else {
            self.abort_restore_in_place(permit, "target-value-decode-failed")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        };
        if !restore_plan_matches_active_session(&plan, &self.session.snapshot())
            || !self
                ...
                .restore_matches(plan.operation_id(), plan.plan_hash(), index)
        {
            self.abort_restore_in_place(permit, "restore-context-changed")
                .await;
            return Err(WriteCoordinatorError::InvalidRestorePermit);
        }
        ...
        let request_id = self.allocate_request_id();
        let context_hash =
            restore_step_context_hash(&plan, &step, &old_engineering, &target_engineering);
        let preparation = DeviceWritePreparation {
            plan_id,
            operation_id: plan.operation_id(),
            request_id,
        ...
        if !self.audit.is_available() {
            self.abort_restore_in_place(permit, "device-audit-unavailable")
                .await;
            self.session.degrade_audit_and_disarm();
            return Err(WriteCoordinatorError::RestoreAuditUnavailable);
        }
        let prepared_token = match self.audit.prepare_device_write(preparation.clone()).await {
            Ok(token) if token.matches_preparation(&preparation) => token,
            Ok(_) | Err(_) => {
                self.abort_restore_in_place(permit, "device-audit-prepare-failed")
                    .await;
                self.session.degrade_audit_and_disarm();
                return Err(WriteCoordinatorError::RestoreAuditUnavailable);
            }
    ...
    async fn abort_restore_in_place(&mut self, permit: &mut RestoreOperationPermit, cause: &str) {
        let plan = permit.plan().clone();
