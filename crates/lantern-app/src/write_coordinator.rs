...
impl WriteCoordinator {
    ...
    pub async fn confirm_write(
        ...
        let token = match self.audit.prepare_device_write(preparation.clone()).await {
            ...
            Ok(_) => {
                self.audit_prepare_failed("audit returned a token bound to different write context");
                return Ok(WriteOutcome::NotExecuted(DecisionOutcome::AuditUnavailable));
            }
            Err(error) => {
        ...
        let authority = ExecutionAuthority::Manual(ConsumedPreparedWritePlan { plan: plan.clone() });
...
fn manual_parameter_allowed(parameter: &ValidatedParameter) -> bool {
    matches!(
        ...
        && parameter.write_function().is_some_and(ModbusFunction::is_write)
}
...
mod tests {
    ...
    use lantern_domain::{
        DeviceFingerprint, DeviceWriteOutcome, DriveState, EngineeringValue, ModbusFunction,
        ModbusTable, OperationId, ParameterId, RawRegisters, RegisterAddress, RegisterBlock,
        RegisterCount, RequestId, SessionId, SlaveId,
    };
    use lantern_profile::{ProfileFormat, parse_and_validate_profile};

    use crate::{
        AuditPort, BusError, BusFuture, ClockPort, PortFuture, ProfileTrustError, ProfileTrustPort,
    ...
    impl WriteBusPort for TestBus {
        fn execute(&self, request: crate::PreparedBusWrite) -> BusFuture<'static, ()> {
            self.writes.lock().expect("writes").push(request.values().clone());
            Box::pin(async { Ok(()) })
        }
    }
    ...
    fn authority_mints_a_width_checked_safety_capability() {
        ...
        let request = WriteCoordinator::test_only()
            ...
            .expect("capability");
        assert_eq!(request.context().class(), crate::RequestClass::SafetyOneShot);
    ...
    async fn operation_step_uses_same_kernel_and_exact_target_without_restore_types() {
        ...
        assert_eq!(authority.expected_old_raw, RawRegisters::new(vec![90]).expect("raw"));
