...
use crate::{MappedAction, UiAction, UiState, fault_primary_parameter, visible_fault_events};
...
pub fn map_fault_key(ui: &UiState, view: &ApplicationView, key: KeyEvent) -> Option<MappedAction> {
    ...
    match key.code {
        KeyCode::Down | KeyCode::Char('j') => {
            (selected + 1 < events.len()).then_some(MappedAction::Ui(UiAction::SelectionNext))
        }
        ...
        KeyCode::Char('a') => events.get(selected).map(|event| {
            MappedAction::Application(Box::new(ApplicationAction::Faults(
                FaultAction::Acknowledge(event.event.event_id),
            )))
        }),
