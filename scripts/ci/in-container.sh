...
set -eu
test "$#" -ge 1
case "$1" in quality) ;; *) echo 'unsupported CI container command' >&2; exit 2 ;; esac
...
podman run --rm --network=none --cpus=4 --memory=6g --pids-limit=1024 \
    ...
    -v "$ci_rustup_home:/rustup:ro" -v "$ci_tools:/ci-tools:ro" \
    -e CARGO_HOME=/cargo -e RUSTUP_HOME=/rustup -e CARGO_NET_OFFLINE=true \
    -e CARGO_BUILD_JOBS=4 -e RUSTUP_TOOLCHAIN=1.97.1 \
