...
result=$?
set -e
observed_seconds=$(( $(date +%s) - started ))
...
jq -e --argjson code "$result" --arg demo "$DEMO" --arg gate "$GATE" \
    ...
    --argjson observed "$observed_seconds" '
    .schema_version == 1 and .status == "pass" and $code == 0
    and (if $demo == "true" then .evidence_kind == "mock" else .evidence_kind == "candidate" end)
    and ([.metrics.cpu_seconds, .metrics.peak_rss_kib, .metrics.iterations,
        .metrics.latency_p95_ms, .metrics.drops, .elapsed_seconds, .requested_seconds]
        | all(type == "number"))
    and (.metrics.cpu_seconds >= 0) and (.metrics.peak_rss_kib > 0)
    and (.metrics.iterations > 0) and (.metrics.latency_p95_ms >= 0)
    and (.metrics.drops >= 0)
    and .requested_seconds == $duration and (.elapsed_seconds >= $duration) and $observed >= $duration
