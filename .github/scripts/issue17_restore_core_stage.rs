...
fn replace_once(path: &str, old: &str, new: &str) {
    ...
    let index = text.find(old).unwrap_or_else(|| panic!("anchor missing in {}: {}", path.display(), &old[..old.len().min(120)]));
    text.replace_range(index..index + old.len(), new);
    fs::write(path, text).unwrap_or_else(|e| panic!("write {}: {e}", path.display()));
}

fn main() {
    let backup_path = Path::new("crates/lantern-app/src/backup.rs");
    let mut backup = fs::read_to_string(backup_path).expect("read backup.rs");
    ...
    fs::write(backup_path, backup).expect("write backup.rs");
...
"###;
    replace_once(
        ...
        &(METHODS.to_owned() + "    async fn execute_once(\n"),
    );
    replace_once(
        "crates/lantern-app/src/write_coordinator.rs",
...
"###;
    replace_once(
        ...
        &(HELPERS.to_owned() + "fn manual_parameter_allowed(parameter: &ValidatedParameter) -> bool {\n"),
    );
}
