...
exec > >(tee /tmp/issues-1-9-v7.log) 2>&1
git config --global --add safe.directory "$GITHUB_WORKSPACE"
git config user.name "vfd-lantern-ci"
git config user.email "actions@users.noreply.github.com"

copy_script() {
  cp "automation/$1" "/tmp/$1"
}

for script in \
  issue7.py patch_issue7_v2.py patch_issue7_v3.py patch_issue7_final.py \
  patch_issue7_error_api.py issue8.py patch_issue8.py patch_issue8_final.py \
  patch_issue8_timing.py patch_issue8_visibility_stats.py issue9.py patch_issue9.py \
  patch_issue9_final.py patch_issue9_borrows.py patch_issue9_completeness.py \
  audit_fixups.py patch_issue1_machete.py patch_issue2_libc.py; do
  copy_script "$script"
done
...
# The pinned libc version belongs to the toolchain baseline (#2), not to #7.
echo '=== compatibility fixup for #2 ==='
issue2="$(git log --format='%H %s' origin/main..HEAD | awk '/\(#2\)$/ {print $1; exit}')"
python3 /tmp/patch_issue2_libc.py
...
python3 /tmp/patch_issue1_machete.py
...
if git log --format='%s' origin/main..HEAD | grep -q '^fixup!'; then
  echo 'fixup commit remained after autosquash' >&2
  exit 1
fi
...
git push --force origin HEAD:agent/issues-1-9-final-candidate-v7
