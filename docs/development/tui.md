...
1. **Port** — render the passive udev snapshot, including stable `/dev/serial/by-id` when available, kernel node, VID/PID, serial, driver, product and presence. `r` requests another passive snapshot. `m` edits a Manual path. A `--device` value is only a prefill; it never opens the device automatically.
...
4. **Summary** — show the exact adapter/profile/link selection. This step is still read-only and the serial port is not open.
...
Manual selection intentionally fabricates no stable ID, VID/PID or serial metadata. The open path still canonicalizes the character device and validates the actual descriptor before constructing the bus actor.
...
During controlled reconnect, `SessionStateMachine` preserves the logical `SessionId` but accepts the transport only if the new Verified identity has the same device fingerprint, profile ID and profile hash. A changed identity faults the session and closes the new transport. The invariant is tested separately from the process-level wizard flow; the simulator's internal `FingerprintChange` field is not treated as hardware-visible evidence.
...
The mandatory CI step `Run connection process E2E` executes a separate real `lantern-sim` process and a separate real `vfd-lantern` process connected through the simulator PTY. `vfd-lantern` runs under a controlled terminal and is driven through the same Connection wizard with normal keys. The matrix covers:
...
The simulator JSONL trace is checked after every process case. It permits only read functions 03/04 and a bounded number of identification requests, providing explicit evidence that the wizard performs no write and starts no hidden telemetry polling. Controlled reconnect/changed-identity behavior is additionally covered by the session/reconnect tests because an internal simulator fingerprint mutation is not itself a bus-observable hardware identity change.
