...
The permanent self-hosted performance gate renders a 120×40 `TestBackend` with eight active channels, four panels and 512 retained points per channel. It uses 40 warm-up frames and 400 measured release-mode frames. The enforced #25 budget is p95 <20 ms and p99 <33 ms. The accepted feature run measured p95 841 µs and p99 877 µs, and the same benchmark remains part of every standard PR gate.
